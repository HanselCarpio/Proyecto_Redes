...
public class EchoClient {
    ...
    public boolean difFiles() {
        try {
            ...
            String[] dirServerFile = Client_TFTP.getWindowClientTFTP().getFiles();
            ...
            if (listAux.size() != 0 && !arrayList.equals(listAux)) {
                arrayList = listAux;
                Client_TFTP.getClient().getOut().println("sincronizar");
    ...
    public void start() {
        String inputLine = "";
        try {
            if ((inputLine = Client_TFTP.getClient().getIn().readLine()) != null) {

                if (inputLine.equalsIgnoreCase("Enviar nombre archivo")) {
                    if (Client_TFTP.getWindowClientTFTP().jTable_FilesTable.getSelectedRow() != -1) {
                        DefaultTableModel model = (DefaultTableModel) Client_TFTP.getWindowClientTFTP().jTable_FilesTable.getModel();
                        String nombre = (String) model.getValueAt(Client_TFTP.getWindowClientTFTP().jTable_FilesTable.getSelectedRow(), 0);
                        Client_TFTP.getClient().getOut().println(nombre);
                        Client_TFTP.getWindowClientTFTP().downloadFile();
                    }
                }
                ...
                if (inputLine.equalsIgnoreCase("Enviar nombre archivo a eliminar")) {
                    if (Client_TFTP.getWindowClientTFTP().jTable_FilesTable.getSelectedRow() != -1) {
                        ...
                        String nombre = (String) model.getValueAt(Client_TFTP.getWindowClientTFTP().jTable_FilesTable.getSelectedRow(), 0);
                        Client_TFTP.getClient().getOut().println(nombre);
                        Client_TFTP.getClient().difFiles();
                    }
                }

                if (inputLine.equalsIgnoreCase("inicio")) {
                    Client_TFTP.getLoginWindow().init();
                    Client_TFTP.getClient().getOut().println("Recibido inicio");
                }
                ...
                if (inputLine.equalsIgnoreCase("acceso")) {
                    Client_TFTP.getWindowClientTFTP().init();
                    Client_TFTP.getClient().getOut().println("Recibido acceso");
                    Client_TFTP.getLoginWindow().dispose();
                }
                if (inputLine.equalsIgnoreCase("Datos incorrectos")) {
                    JOptionPane.showMessageDialog(Client_TFTP.getWindowClientTFTP(), "Introduzca datos validos de usuario");
                }

                if (inputLine.equalsIgnoreCase("Enviar directorios")) {
                    Client_TFTP.getClient().getOut().println("Recibido: Enviar directorios");
                    int tam = Integer.parseInt(Client_TFTP.getClient().getIn().readLine());
                    Client_TFTP.getClient().getOut().println("Recibido tam");
                    System.out.println("tamaño:  " + tam);
                    Client_TFTP.getClient().getOut().println("tam recibido");

                    String[] directorios = new String[tam];
                    for (int j = 0; j < tam; j++) {
                        directorios[j] = Client_TFTP.getClient().getIn().readLine();
                    }
                    for (int j = 0; j < directorios.length; j++) {
                        System.out.println("Directorios: " + directorios[j]);
                    }

                    Client_TFTP.getWindowClientTFTP().setDirectories(directorios);
                }
...
class ReciveAndSendServer extends Thread {
    ...
    public void run() {
        while (true) {
            Client_TFTP.getClient().start();
        }
    }

    public boolean sendMessage(String inputLine) {
        if (inputLine.equals("1")) {
            Client_TFTP.getClient().getOut().printf("Enviar directorios");
...
class Sincronize extends Thread {
    ...
    public void run() {
        while (true) {
            System.getenv(Client_TFTP.getClient().getAbsolutepath());

            if (Client_TFTP.getClient().getAbsolutepath() != "") {
                if (Client_TFTP.getClient().difFiles()) {
                    Client_TFTP.getClient().getOut().println("" + Client_TFTP.getClient().getArrayList().size());
                }
            }
